...
public class Httpuyghurpeoplecomcn extends WebCrawler {
   ...
   // private final static String URL_PREFIX = "http://ti.tibet3.com/";
	...
	//中文站点前缀
	private final static String URL_PREFIX = "http://uyghur.people.com.cn/";
    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    Elements contents_html = doc.select("span#zoom");
                    if(!contents_html.isEmpty()){
                   	//标题部分
                    String title =doc.select("h1#p_title").text();
                    //正文部分内容
                    String content = doc.select("span#zoom").text();                  
                    //发布时间
                    String timeofpublish =doc.select("strong#p_publishtime").text();
                    // 本url域名地址
                    String domainname = page.getWebURL().getURL();
                    //提供网站地址 
                    String provider= URL_PREFIX;
                    //作者
                    String author=doc.select("strong#p_origin").text();
                    ...
                    Elements E_image=doc.select("div.wb_8 table tbody tr img");
                    System.out.println("***************************************");
                    System.out.println(doc.select("div.wb_8 table tbody tr img").attr("src"));
                    System.out.println("***************************************");
                    i++;
                    
                    //获取的发布时间并转换成simpleDataFormat格式                     
                    //模版 09:38, September 19, 2014，里面存在英文所以要locale 英文
                    String publishtimeFormation="yyyy/MM/dd HH:mm";
