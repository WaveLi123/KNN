...
public class UyghurnewscnParse extends WebCrawler {
   ...
   // private final static String URL_PREFIX = "http://ti.tibet3.com/";
	...
	//朝文站点前缀
	private final static String URL_PREFIX = "http://uyghur.news.cn";
    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    Elements contents_html = doc.select("div.wenzhang");
                    if(!contents_html.isEmpty()){
                   	//标题部分
                    String title =doc.select("div.biaoti").text();
                    //正文部分内容
                    String content = doc.select("div.wenzhang").text();
                    //解决 2011~2013 网页规则与2014 不统一的情况
                    if(content.isEmpty()||content.equalsIgnoreCase(title)) content = doc.select(".wb_4 p").text();                   
                    //发布时间
                    String timeofpublish =doc.select("div.laiyuan .time").text();
                    // 本url域名地址
                   // String domainname =templateUrl;
                    ...
                    //作者
                    String author=doc.select("div.laiyuan").text().substring(0, doc.select("div.laiyuan").text().length()-doc.select("div.laiyuan .time").text().length());
...
//       				String timeofdownload=df.format(new Date()).toString();
                  ...
                  System.out.println("获取格式检查"+dateOfPublish+"\n");
                    ...
                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");//设置日期格式
       				...
       				String timeofdownload=df.format(new Date()).toString();
       			  //添加附加信息
                    String chinesename="新华网维语版";
                    String pinyin="xhwwyb";
                    ...
                    //获取图片对应url地址
                    Elements E_image=doc.select(".wenzhang p img");
