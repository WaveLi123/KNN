    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    //发布时间
                    String timeofpublish = null; 
                    if(doc.select("#p_publishtime").text().length() > 30){
                    	timeofpublish =doc.select("#p_publishtime").text().substring(14, doc.select("#p_publishtime").text().length());
                    }
                    else{
                    	timeofpublish =doc.select("#p_publishtime").text();
                    }
              ...
              //      System.out.println("获取格式检查"+dateOfPublish+"\n");
                    ...
                    //下载时间                   
                    SimpleDateFormat df = new SimpleDateFormat("HH:mm, MMMMM dd, yyyy",Locale.ENGLISH);//设置日期格式
