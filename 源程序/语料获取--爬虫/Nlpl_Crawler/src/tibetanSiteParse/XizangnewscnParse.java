...
public class XizangnewscnParse extends WebCrawler {
   ...
   // private final static String URL_PREFIX = "http://ti.tibet3.com/";
	...
	//朝文站点前缀
	private final static String URL_PREFIX = "http://xizang.news.cn";
    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    Elements contents_html = doc.select(".style");
                    if(!contents_html.isEmpty()){
                   	//标题部分
                    String title =doc.select(".STYLE1Copy").text();
                    //正文部分内容
                    String content = doc.select(".style").text();
                    //解决 2011~2013 网页规则与2014 不统一的情况
                    if(content.isEmpty()||content.equalsIgnoreCase(title)) content = doc.select(".wb_4 p").text();                   
                    //发布时间
                    //String timeofpublish =doc.select(".STYLE5").text();
                    
                    //changed by jkd,27th Oct in 2014
                    String timeofpublish =doc.select(".STYLE5").text().substring(0,10);
                    /////////////////////////////////
                   ...
                   // String domainname =templateUrl;
                    ...
                    //作者
                    String author=doc.select(".STYLE5 .heise").text();;
...
//                    //模版 :2014-10-27  ，里面存在英文所以要locale 英文	Site:http://xizang.news.cn
//                    SimpleDateFormat dfpt = new SimpleDateFormat("yyyy-mm-dd",Locale.ENGLISH);//设置日期格式                    
...
//       				String timeofdownload=df.format(new Date()).toString();
                  //获取的发布时间并转换成simpleDataFormat格式                     
                    ...
                    String publishtimeFormation="yyyy-MM-dd";
                    ...
                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");//设置日期格式
       				...
       				String timeofdownload=df.format(new Date()).toString();
       			  //添加附加信息
                    String chinesename="新华网多语种西藏频道";
                    String pinyin="xhwdyzxzpd";
                    ...
                    //获取图片对应url地址
                    Elements E_image=doc.select(".style .bai13 img");
