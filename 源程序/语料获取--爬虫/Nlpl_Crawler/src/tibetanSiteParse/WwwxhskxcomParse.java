...
public class WwwxhskxcomParse extends WebCrawler {
   ...
   // private final static String URL_PREFIX = "http://ti.tibet3.com/";
	...
	//朝文站点前缀
	private final static String URL_PREFIX = "http://www.xhskx.com";
    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    Elements contents_html = doc.select(".newscontent");
                    if(!contents_html.isEmpty()){
                   	//标题部分
                    String title =doc.select(".newstitle,.newstitle2").text();
                    //正文部分内容
                    String content = doc.select(".newscontent p,.newscontent span,.tc-content-wrap").text();
                    //解决 2011~2013 网页规则与2014 不统一的情况
                    if(content.isEmpty()||content.equalsIgnoreCase(title)) content = doc.select(".wb_4 p").text();                   
                    //发布时间
                    String timeofpublish =doc.select(".newsinfo,.tc-head-wrap h2").text();
                    // 本url域名地址
                   // String domainname =templateUrl;
                    ...
                    //作者
                    String author=doc.select(".editor,.newscontent .editor").text();;
...
//       				String timeofdownload=df.format(new Date()).toString();
                  ...
                  System.out.println("获取格式检查"+dateOfPublish+"\n");
                    ...
                    SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss");//设置日期格式
       				...
       				String timeofdownload=df.format(new Date()).toString();
       			  //添加附加信息
                    String chinesename="指闻天下藏文模块";
                    String pinyin="zwtxzwmk";
                    ...
                    //获取图片对应url地址
                    Elements E_image=doc.select(".newscontent p img");
                    ...
                    for(int j=0;it.hasNext();j++)
	                  ...
	                  //  if(it.hasNext())
	                   ...
	                   // System.out.println(imageAdress);
	                    ...
	                    //changed by jkd,29th Oct in 2014
	                    String pathAndimageFileName=path+"\\"+imageFileName+".jpg";
