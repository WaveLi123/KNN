...
public class Httptitibetcn extends WebCrawler {
   ...
   // private final static String URL_PREFIX = "http://ti.tibet3.com/";
	...
	//朝文站点前缀
	private final static String URL_PREFIX = "http://tb.tibet.cn/";
    ...
    public void visit(Page page) {   
            ...
            if (page.getParseData() instanceof HtmlParseData) {
            	   //则把page对象解析出来的数据强制转换为htmlparsedata
                    ...
                    Elements contents_html = doc.select("p.MsoNormal");
                    if(!contents_html.isEmpty()){
                   	//标题部分
                    String title =doc.select("td.lan_20,td.hei_c1").text();
                    //正文部分内容
                    String content = doc.select("p.MsoNormal").text();
                    //解决 2011~2013 网页规则与2014 不统一的情况
                    if(content.isEmpty()||content.equalsIgnoreCase(title)) content = doc.select(".wb_4 p").text();                   
                    //发布时间                    
                    String timeofpublish =doc.select("[height=20]").text().substring(10, 20);
                    // 本url域名地址
                   // String domainname =templateUrl;
                    ...
                    //作者
                    String author=doc.select("[height=20]").text().substring(20);
                    ...
                    Elements E_image=doc.select("p.MsoNormal span img");
                    System.out.println("**********************************");
                    System.out.println(doc.select("p.MsoNormal span img").attr("src"));
                    System.out.println("**********************************");
                    ...
                    SimpleDateFormat dfpt = new SimpleDateFormat("yyyy-MM-dddd",Locale.ENGLISH);//设置日期格式
